...
knifeSwooshSound = loadSound("knifeSwoosh.mp3");//carregar o som quando corta as frutas <<<<
  //carregue o som aqui
}
function setup() {
...
function draw() {
  ...
  if(gameState===PLAY){
    ...
    if(fruitGroup.isTouching(knife)){
      ...
      knifeSwooshSound.play(); //toca o som <<<<
      score = score + 2; //aumenta a pontuação <<<
...
function fruits(){
  if(World.frameCount%80===0){
    ...
    fruit.velocityX=-(7+(score/4)); //muda a velocidade <<<
    }
    else
    {
      if(position==2){
